// pages/admin/locationManagement/locationManagement.js
...
//   },
  ...
  async loadLocations() {
    ...
    try {
      console.log('开始加载打卡地列表...')
      const res = await api.request({
        url: '/admin/checkin-locations',
        method: 'GET',
        data: { status: 1 }  // 传递 status 参数
      })
      
      console.log('打卡地列表响应:', res)
      ...
      if (res.code === 200 && res.data) {
        ...
        console.log('处理后的打卡地列表:', locations)
        this.setData({ locations })
      } else {
        console.log('响应异常或无数据')
        this.setData({ locations: [] })
      }
    } catch (error) {
...
//   },
  onProjectChange(e) {
    ...
    if (this.data.projects && this.data.projects[index]) {
      ...
      console.error('项目数据错误:', this.data.projects);
      wx.showToast({
        title: '项目数据错误',
        icon: 'none'
      })
  ...
  async submitEdit() {
    ...
    try {
      const res = await api.request({
        url: `/admin/checkin-location/${this.data.currentLocation.id}`,
        method: 'PUT',
        data: this.data.formData
      })
  ...
  async doDeleteLocation(id) {
    ...
    try {
      const res = await api.adminDeleteCheckinLocation(id)
