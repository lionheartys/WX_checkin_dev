// pages/manager/makeupAudit/makeupAudit.js
...
Page({
  ...
  async loadApplications() {
    ...
    try {
      const res = await api.managerGetMakeupApplications(this.data.activeTab)
  ...
  async updateCounts() {
    try {
      ...
      for (const status of statuses) {
        const res = await api.managerGetMakeupApplications(status)
        if (res.code === 200) {
  ...
  async handleAudit(status) {
    ...
    try {
      const res = await api.managerAuditMakeup({
